...
const ArtistCards = () => (
  <div className="grid gap-6 lg:grid-cols-4">
    {artists.map((artist, index) => (
        <div
        key={index}
        className="rounded-lg border border-white border-1 bg-[#0f0f0f] p-4 shadow-md rounded-xl"
        style={{
          boxShadow:
      ...
      >
        <Image
          alt={artist.name}
          className="aspect-square w-full rounded-lg object-cover brightness-75 grayscale"
          height="400"
          src={artist.image}
          width="400"
        />
